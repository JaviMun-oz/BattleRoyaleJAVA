...
public class GameEngine {
    ...
    public void setupGame() {
        ...
        List<Character> chars = createCharacters();
        ...
        for (int i = 0; i < numHuman; i++) {
            ...
            Character chosen = chooseCharacter(chars);
            chars.remove(chosen);

            Tool chosenTool = chooseTool(tools);
            Tool playerTool = cloneToolByName(chosenTool.getName());

            players.add(new Player(name, true, chosen, playerTool));
        }

        // AI players
        for (int i = 0; i < numAI; i++) {
            String name = "AI-" + (i+1);
            Character c = chars.get(rnd.nextInt(chars.size()));
            chars.remove(c);
            Tool t = tools.get(rnd.nextInt(tools.size()));
            Tool playerTool = cloneToolByName(t.getName());
            players.add(new Player(name, false, c, playerTool));
        }
    ...
    private List<Character> createCharacters() {
        ...
        list.add(new BalancedCharacter("Bal-2"));
        list.add(new SpeedCharacter("Spd-2"));
        list.add(new DamageCharacter("Dmg-2"));
        return list;
    }
