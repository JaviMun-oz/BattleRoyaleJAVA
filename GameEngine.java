...
public class GameEngine {
    ...
    public void setupGame() {
        ...
        int numHuman = askInt("Number of human players (0-4): ", 0, 4);
    ...
    private Character chooseCharacter(List<Character> candidates) {
        ...
        for (int i = 0; i < candidates.size(); i++) {
            ...
            System.out.printf("[%d] %s (Atk:%d Def:%d Spd:%d HP:%d)\n", i+1, c.getClassName(), c.getBaseAttack(), c.getBaseDefense(), c.getBaseSpeed(), c.getBaseHealth());
        }
    ...
    private void resolveBattle(Player a, Player b) {
        ...
        damage = applyDifficultyDamage(winner, loser, damage);
    ...
    private int applyDifficultyDamage(Player attacker, Player defender, int baseDamage) {
        // Hard: AI deals double damage
        if (!attacker.isHuman() && difficulty == 3) {
            return baseDamage * 2;
        }

        // Easy: AI takes double damage
        if (!defender.isHuman() && difficulty == 1) {
            return baseDamage * 2;
        }

        // Normal or no modifier
        return baseDamage;
    }
    ...
    private void endGame() {
        ...
        if (winner.isHuman()) System.out.println("https://youtu.be/dQw4w9WgXcQ?si=4Xt52VSOmm9C23NL");
        else System.out.println("No winner (all dead)");

        System.out.print("Save result + operations log to a file? (Y/N): ");
        String save = scanner.nextLine().trim();

        if (save.equalsIgnoreCase("Y")) {
            System.out.print("Enter filename (e.g., result.txt): ");
            String filename = scanner.nextLine().trim();

            try {
                FileUtils.saveGameResult(filename, winner, log);
